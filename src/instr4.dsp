...
degI2 = 0-hslider("degre rms", 4, -180, 180 , 1): si.smoo;
//-----------------------------------------------------------------------------
...
makeMidSide(i,degI,in) = in : mspan(pp,(degI*frequenzaPicchi(timePeak,pkTresh,1.4,1)*i+((movingAverageRMS(0.5)*20)*degI2)):si.smoo);
makeMidSide2(i,degI,in) = in : mspan(pp,(degI*(i*.3)+degI2*frequenzaPicchi(timePeak,pkTresh,1.4,1)):si.smoo);
//----------------
...
//============================
//
degI1 = 0-hslider("degre peak", -90, -180, 180 , 1) : si.smoo;
scaleFreq = hslider("freq SCALER", 1.2, 0.1, 1.6, 0.01): si.smoo;
pp = hslider("polar pattern", .5, 0, 1 , 0.01);
timeReverb = hslider("time reverb", 1, 1, 12, 0.01): si.smoo;
lfoFreq = hslider("freq LFO", 2.6, .2, 10, 0.01): si.smoo;
//============================
inGain =hslider("[1]gain In", 1, 0, 4, 0.01): si.smoo;
gainReverb = hslider("gain Rev", 0.11, 0, 1, 0.001): si.smoo;

smistatoreChord(amp, in) = in, si.bus(6) : chord2(mappingQfilter6(1.5),noiseFbkBank2(fDev,scaleFreq,0.01,0.5), degI1,makeMidSide2,timeReverb,funtSec,gainReverb);
